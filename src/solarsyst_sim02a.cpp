// October 01, 2024: solarsyst_sim02a.cpp
// Like solarsyst_sim01a.cpp, but uses integrate_orbit03LD() rather than
// integrate_orbit04LD(). The expectation is that this may be slower,
// but will be less sensitive to roundoff error. It also does not require
// an mjdend parameter, and furthermore it should be acceptable to use
// a non-integer mjdstart that is not necessarily earlier than all
// of the image times, since integrate_orbit03LD() can handle backward
// integration as well as forward.
//
...
{
  cerr << "Usage: solarsyst_sim02a -cfg configfile -ranseed random_number_seed -images imfile -imrad image radius(deg) -mjdstart mjdstart -astromerr 1-D astrometric error (arcsec) -obslon obslon -plxcos plxcos -plxsin plxsin -simnum simnum -outfile outfile \n";
}
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
	...
	// Read (and discard) default ending MJD
	status=readconfigLD(instream1,&ldval);
	while(status==1) {
	  // The line we have just read is a pure comment line,
	  // so we just want to skip to the next one.
	...
	if(status<0) {
	  cerr << "Error reading config file\n";
	  return(1);
	} else cout << "Default ending MJD (which will not be used) read as " << ldval << "\n";
  ...
  for(simct=0; simct<simnum; simct++) {
   ...
   // Convert from AU and AU/day to km and km/sec
    ...
    planetposvel01LD(mjdstart,polyorder,planetmjd,Sunpos,Sunvel,Sunposnow,Sunvelnow);
    outpos.x += Sunposnow.x;
    outpos.y += Sunposnow.y;
    outpos.z += Sunposnow.z;
    outvel.x += Sunvelnow.x;
    outvel.y += Sunvelnow.y;
    outvel.z += Sunvelnow.z;
      
    // 8. Perform n-body integration beginning with the state vectors
    // produced by Kepler2dyn
    integrate_orbit03LD(polyorder, planetnum, planetmjd, planetmasses, planetpos, imageMJD, outpos, outvel, mjdstart, targpos, targvel);
    if(imnum != long(imageMJD.size()) || imnum != long(targpos.size()) || imnum != long(targvel.size())) {
      cerr << "ERROR: vector length mismatch in integrated orbit: imnum = " << imnum << ",\n";
      cerr << "while vector lengths are " << imageMJD.size() << ", " << targpos.size() << ", and " << targvel.size() << "\n";
      return(3);
    }
    for(imct=0;imct<imnum;imct++)
      {
	imagepos.x = targpos[imct].x;
	imagepos.y = targpos[imct].y;
	imagepos.z = targpos[imct].z;
	imagevel.x = targvel[imct].x;
	imagevel.y = targvel[imct].y;
	imagevel.z = targvel[imct].z;
