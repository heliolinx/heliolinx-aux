// January 07, 2026: MOIDfind01b.cpp
...
{
  cerr << "Usage: MOIDfind01b -cfg configfile -mjdstart mjdstart -mjdend mjdend -mpcorb Epoch M Peri Node Incl e n a -outfile outfile -verbose verbosity\n";
}
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
	...
	return(1);
      ...
      if(i+1 < argc) epochstring=argv[++i];
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) M=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) Peri=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) Node=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) Incl=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) e=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) n=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
	}
      if(i+1 < argc) a=stold(argv[++i]);
      else {
	  cerr << "mpcorb keyword supplied with fewer than the required 8 arguments\n";
	  show_usage();
	  return(1);
  ...
  if(refpoint<0) {
    ...
    if(j<0 || planet_forward_mjd[j]>mjdref) {
      cerr << "ERROR: failed to find a point near mjdref = " << mjdref << "\n";
      return(5);
    } else refpoint=j;
  ...
  if(minvpt>2) mjdnow = targMJD[minvpt-2];
  else mjdnow = targMJD[0];
  if(minvpt<long(targMJD.size()-2)) mjdmax = targMJD[minvpt+2];
  else mjdmax = targMJD[targMJD.size()-1];
  while(mjdnow<mjdmax) {
    // Interpolate Earth position
    Earthnow = {};
