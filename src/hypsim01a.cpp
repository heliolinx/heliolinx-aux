...
{
  cerr << "Usage: hypsim01a -cfg configfile -ranseed random_number_seed -times timefile -hyp hypothesis_file -unbound 0 -mjdref mjdref -mjdstart mjdstart -mjdend mjdend -astromerr 1-D astrometric error (arcsec) -simnum simnum -outfile outfile \n";
}
...
int main(int argc, char *argv[])
{
  ...
  outstream1 << "stringID a e incl q Q mjdref n sundist obsdist sunelong phaseang mjdobs outRA outDec Ast-Sun(J2000x)(km) Ast-Sun(J2000y)(km) Ast-Sun(J2000z)(km) Ast-Sun(J2000vx)(km/s) Ast-Sun(J2000vy)(km/s) Ast-Sun(J2000vz)(km/s) perfectRA perfectDec\n";
  ...
  for(hypct=0; hypct<hypnum; hypct++) {
	...
	if(dist > 90.0l-minalt) {
	  // Object is below the functional horizon
	  goodobject=0;
	}
      }
      if(goodobject==1) {
	...
	for(timect=0;timect<timenum;timect++) {
	  ...
	  double q = a*(1.0L-e);
	  double Q = a*(1.0L+e);
	  long double cosphase = dotprod3LD(obs_to_targ,obs_to_sun)/obsfromsun; // obs_to_targ was normalized already.
	  if(cosphase>1.0L) {
	    cout << "WARNING: trying to take arccos of 1.0 + " << cosphase-1.0L << "\n";
	    sunelong=0.0L;
	  } else if(cosphase<-1.0L) {
	  ...
	  outstream1 << fixed << setprecision(6) << a/AU_KM << " " << e << " " << incl << " " << q/AU_KM << " " << Q/AU_KM << " " << mjdref << " " << n*SOLARDAY*DEGPRAD << " ";
	  ...
	  outstream1 << fixed << setprecision(6) << outRA << " " << outDec << " " << alt << "\n";
