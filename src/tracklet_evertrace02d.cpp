// January 22, 2026: tracklet_evertrace02d.cpp:
// Uses heliocentric coords for the rescue Keplerian fit.
// Like tracklet_evertrace02a.cpp, but attempts a rescue Keplerian fit
// in the case where the full evertrace01() fit failed.
//
// Based on tracklet_evertrace01c.cpp, but processes multiple ephemeris
// files for one read of the input make_tracklets files.
// The input file must have, on each line, the names of three other files:
// first an input observation file in hldet format; second an input
// representative MCMC output file produced, e.g., by MCMC_everhart01a.cpp,
// and third an output filename for the augmented linkage, which will
// be written only if at least one new tracklet is successfully added.
// 
// January 20, 2026: tracklet_evertrace01c.cpp: Adds important option
// to remove second-round deduplication, but defaults to leaving it in.
// There are also other improvements to the logic. As far as I know,
// this version is better in every way than tracklet_evertace01b.cpp.
// 
// Description of ancestor program tracklet_evertace01b.cpp:
// Like tracklet_evertrace01a.cpp, but performs tracklet redundancy
// check much faster, using MJD alone.
//
...
int main(int argc, char *argv[])
{
  ...
  vector <double> kep_statevec;
  vector <double> kepout_statevec;
  ...
  vector <vector <double>> targ_tempstate;
  ...
  int kepnum,simptype;
  simptype=kepnum=1;
  vector <double> KepfitRA, KepfitDec, Kepfitresid;
  double stateMJD = 0.0;
  double kep_chisq = 0.0;
  double kep_astromRMS = 0.0;
  ...
  while(i<argc) {
    ...
    } else if(string(argv[i]) == "-dedup" || string(argv[i]) == "-dup" || string(argv[i]) == "-duprounds" || string(argv[i]) == "-dedup_rounds" || string(argv[i]) == "--dedup" || string(argv[i]) == "--dedup_rounds" || string(argv[i]) == "--num_dedup_rounds") {
      if(i+1 < argc) {
	//There is still something to read;
	dedup=stoi(argv[++i]);
	i++;
	...
	cerr << "Number of deduplication rounds keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
    } else {
  ...
  queryrange *= sqrt(3.0);
  cout << "queryrange = " << queryrange << "\n";
  outstream1 << "queryrange = " << queryrange << "\n";
  ...
  outstream1 << "Created a k-d tree with " << kdtree.size() << " branches\n";

  // READ AND LOOP OVER LINES OF INPUT FILE
  instream2.open(infile);
  while(!instream2.fail() && !instream2.bad() && !instream2.eof()) {
    obsfile="";
    repfile="";
    outfile="";
    badread=0;
	...
	outstream1 << "can't open input file " << repfile << "\n";
	return(1);
      }
      reachedeof = 0;
      while(reachedeof==0) {
	...
	return(1);
      ...
      kep_statevec = kepout_statevec = {};
      kepnum = obsMJD.size();
      // Calculate geocentric distance at start and end of observations sequence
      targ_tempstate={};
      stateMJD=0.0;
      status = obsint_everuse01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, starting_statevecs[0], mjdstart, mjd_sv_vec[0], mjdend, obsMJD, targ_tempstate, timestep, hnum, hspace, verbose);
      geodist1 = sqrt(DSQUARE(targ_tempstate[0][0] - observer_statevecs[0][0]) + DSQUARE(targ_tempstate[0][1] - observer_statevecs[0][1]) + DSQUARE(targ_tempstate[0][2] - observer_statevecs[0][2])) / AU_KM;
      geodist2 = sqrt(DSQUARE(targ_tempstate[kepnum-1][0] - observer_statevecs[kepnum-1][0]) + DSQUARE(targ_tempstate[kepnum-1][1] - observer_statevecs[kepnum-1][1]) + DSQUARE(targ_tempstate[kepnum-1][2] - observer_statevecs[kepnum-1][2])) / AU_KM;
      cout << "\nLaunching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
      outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
      observer_heliostate = observer_statevecs;
      for(obsct=0;obsct<long(observer_statevecs.size());obsct++) {
	// Calculate the sun's position at the same time, noting that planetpos02 assumes input is TDB
	planetpos02(obsMJD[obsct]+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
	// Convert obsstate from barycentric to heliocentric coords.
	for(k=0;k<6;k++) observer_heliostate[obsct][k] -= sunstate[k];
      }
      KepfitRA = KepfitDec = Kepfitresid = kep_statevec = kepout_statevec = {};
      kep_chisq = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_heliostate, obsMJD, obsRA, obsDec, sigastrom, KepfitRA, KepfitDec, Kepfitresid, kep_statevec, stateMJD, itnum, verbose);
      Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, mjd_sv_vec[0], kepout_statevec, verbose);
      // Convert Keplerian output vector from heliocentric to barycentric coordinates.
      planetpos02(mjd_sv_vec[0]+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
      for(k=0;k<6;k++) kepout_statevec[k] += sunstate[k];
      
      kep_astromRMS = 0.0;
      for(i=0;i<long(obsMJD.size());i++) kep_astromRMS += Kepfitresid[i]*Kepfitresid[i];
      kep_astromRMS  = sqrt(kep_astromRMS / double(obsMJD.size()-1));
      cout << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";
      outstream1 << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";

      out_statevec = {};
      fitRA = fitDec = {};
      chisq = astromRMS = LARGERR;
	...
	cout << "Rescue Keplerian fit returns chisq, RMS = " << chisq << " " << astromRMS << "\n";
	outstream1 << "Rescue Keplerian fit returns chisq, RMS = " << chisq << " " << astromRMS << "\n";
      }
	...
	cout << "Ephemeris point " << i << ", MJD " << ephMJD[i] << ", sky spread is " << maxdist << " deg, and velocity spread is " << maxvdist << " deg/day\n";
      ...
      outstream1 << "Preparing to search for matches among " << trk_dindvec.size() << " tracklets at " << ephMJD.size() << " different times, for " << repnum << " orbit clones\n";
      for(i=0;i<long(ephMJD.size());i++) {
	...
	outstream1 << "No matching tracklets were found\n";
	outstream1 << "End of analysis for file set " << obsfile << " " << repfile << " " << outfile << "\n";
	continue;
      }
	...
	cout << "User has chosen to skip the second round of de-duplication. Proceeding with " << matching_trkind.size() << " total tracklets\n";
	outstream1 << "User has chosen to skip the second round of de-duplication. Proceeding with " << matching_trkind.size() << " total tracklets\n";
      }
	...
	if(trkvec_temp.size()>0) {
	  ...
	  // Perform experimental Keplerian fit, to which we will revert if necessary to rescue.
	  kepnum = obsMJD2.size();
	  // Calculate geocentric distance at start and end of observations sequence
	  targ_tempstate={};
	  stateMJD=0.0;
	  status = obsint_everuse01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, master_statevec, mjdstart, master_mjd, mjdend, obsMJD2, targ_tempstate, timestep, hnum, hspace, verbose);
	  geodist1 = sqrt(DSQUARE(targ_tempstate[0][0] - observer_statevecs2[0][0]) + DSQUARE(targ_tempstate[0][1] - observer_statevecs2[0][1]) + DSQUARE(targ_tempstate[0][2] - observer_statevecs2[0][2])) / AU_KM;
	  geodist2 = sqrt(DSQUARE(targ_tempstate[kepnum-1][0] - observer_statevecs2[kepnum-1][0]) + DSQUARE(targ_tempstate[kepnum-1][1] - observer_statevecs2[kepnum-1][1]) + DSQUARE(targ_tempstate[kepnum-1][2] - observer_statevecs2[kepnum-1][2])) / AU_KM;
	  cout << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	  outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	  observer_heliostate = observer_statevecs2;
	  for(obsct=0;obsct<long(observer_statevecs2.size());obsct++) {
	    // Calculate the sun's position at the same time, noting that planetpos02 assumes input is TDB
	    planetpos02(obsMJD2[obsct]+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
	    // Convert from barycentric to heliocentric coords.
	    for(k=0;k<6;k++) observer_heliostate[obsct][k] -= sunstate[k];
	  }
	  KepfitRA = KepfitDec = Kepfitresid = kep_statevec = kepout_statevec = {};
	  kep_chisq = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_heliostate, obsMJD2, obsRA2, obsDec2, sigastrom2, KepfitRA, KepfitDec, Kepfitresid, kep_statevec, stateMJD, itnum, verbose);
	  Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, master_mjd, kepout_statevec, verbose);
	  // Convert Keplerian output vector from heliocentric to barycentric coordinates.
	  planetpos02(master_mjd+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
	  for(k=0;k<6;k++) kepout_statevec[k] += sunstate[k];
 	  kep_astromRMS = 0.0;
	  for(i=0;i<long(obsMJD.size());i++) kep_astromRMS += Kepfitresid[i]*Kepfitresid[i];
	  kep_astromRMS  = sqrt(kep_astromRMS / double(obsMJD2.size()-1));
	  cout << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";
	  outstream1 << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";
	    
	  // Perform main fit usng evertrace01
	  out_statevec = {};
	  bestRA = bestDec = {};
	  ...
	  if(status!=0) {
	    ...
	    bestresid={};
	    for(obsct=0;obsct<long(obsRA2.size());obsct++) bestresid.push_back(3600.0 * distradec01(obsRA2[obsct],obsDec2[obsct],bestRA[obsct],bestDec[obsct]));
	  }
	...
	for(twinfitct=0;twinfitct<twinfitnum;twinfitct++) {
	  ...
	  for(matchct=twinfitct+1;matchct<allfitnum;matchct++) {
	    ...
	    if(trkvec_temp.size()<=0) {
	      ...
	      // Launch experimental Keplerian fit
	      kepnum = obsMJD3.size();
	      // Calculate geocentric distance at start and end of observations sequence
	      targ_tempstate={};
	      stateMJD=0.0;
	      status = obsint_everuse01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, master_statevec, mjdstart, master_mjd, mjdend, obsMJD3, targ_tempstate, timestep, hnum, hspace, verbose);
	      geodist1 = sqrt(DSQUARE(targ_tempstate[0][0] - observer_statevecs3[0][0]) + DSQUARE(targ_tempstate[0][1] - observer_statevecs3[0][1]) + DSQUARE(targ_tempstate[0][2] - observer_statevecs3[0][2])) / AU_KM;
	      geodist2 = sqrt(DSQUARE(targ_tempstate[kepnum-1][0] - observer_statevecs3[kepnum-1][0]) + DSQUARE(targ_tempstate[kepnum-1][1] - observer_statevecs3[kepnum-1][1]) + DSQUARE(targ_tempstate[kepnum-1][2] - observer_statevecs3[kepnum-1][2])) / AU_KM;
	      cout << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	      outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	      observer_heliostate = observer_statevecs3;
	      for(obsct=0;obsct<long(observer_statevecs3.size());obsct++) {
		// Calculate the sun's position at the same time, noting that planetpos02 assumes input is TDB
		planetpos02(obsMJD3[obsct]+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
		// Convert from barycentric to heliocentric coords.
		for(k=0;k<6;k++) observer_heliostate[obsct][k] -= sunstate[k];
	      }
	      KepfitRA = KepfitDec = Kepfitresid = kep_statevec = kepout_statevec = {};
	      kep_chisq = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_heliostate, obsMJD3, obsRA3, obsDec3, sigastrom3, KepfitRA, KepfitDec, Kepfitresid, kep_statevec, stateMJD, itnum, verbose);
	      Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, master_mjd, kepout_statevec, verbose);
	      // Convert Keplerian output vector from heliocentric to barycentric coordinates.
	      planetpos02(master_mjd+TTDELTAT/SOLARDAY,5,Sun_mjd,Sun_statevecs,sunstate);
	      for(k=0;k<6;k++) kepout_statevec[k] += sunstate[k];
	      kep_astromRMS = 0.0;
	      for(i=0;i<long(obsMJD.size());i++) kep_astromRMS += Kepfitresid[i]*Kepfitresid[i];
	      kep_astromRMS  = sqrt(kep_astromRMS / double(obsMJD3.size()-1));
	      cout << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";
	      outstream1 << "Experimental Keplerian fit returns chisq, RMS = " << kep_chisq << " " << kep_astromRMS << "\n";

	      // Launch primary fit using evertrace01()
	      out_statevec = {};
	      bestRA = bestDec = {};
	      bestchi = astromRMS = LARGERR;
		...
		if(astromRMS<bestRMS && fabs(twinfitct_MJD-matchct_MJD) > MINTRACKSEP) {
		  ...
		  cout << "The fit is the best found so far, but this tracklet pair cannot serve \n";
		  cout << "as the primary pair because they are too close in time (" << fabs(twinfitct_MJD-matchct_MJD) << " days)\n";
		  outstream1 << "The fit is the best found so far, but this tracklet pair cannot serve \n";
		  outstream1 << "as the primary pair because they are too close in time (" << fabs(twinfitct_MJD-matchct_MJD) << " days)\n";
		} else {
	...
	continue;
      ...
      targ_tempstate={};
      stateMJD=0.0;
      status = obsint_everuse01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, master_statevec, mjdstart, master_mjd, mjdend, obsMJD, targ_tempstate, timestep, hnum, hspace, verbose);
      geodist1 = sqrt(DSQUARE(targ_tempstate[0][0] - observer_statevecs[0][0]) + DSQUARE(targ_tempstate[0][1] - observer_statevecs[0][1]) + DSQUARE(targ_tempstate[0][2] - observer_statevecs[0][2])) / AU_KM;
      geodist2 = sqrt(DSQUARE(targ_tempstate[kepnum-1][0] - observer_statevecs[kepnum-1][0]) + DSQUARE(targ_tempstate[kepnum-1][1] - observer_statevecs[kepnum-1][1]) + DSQUARE(targ_tempstate[kepnum-1][2] - observer_statevecs[kepnum-1][2])) / AU_KM;
      cout << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
      outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	...
	if(i==0 || outdetvec[i].MJD!=outdetvec[i-1].MJD || outdetvec[i].RA!=outdetvec[i-1].RA || outdetvec[i].Dec!=outdetvec[i-1].Dec) {
	  ...
	  outstream2 << fixed << setprecision(4) << outdetvec[i].mag << ",";
	  outstream2 << fixed << setprecision(2) << outdetvec[i].trail_len << "," << outdetvec[i].trail_PA << ",";
	  outstream2 << fixed << setprecision(4) << outdetvec[i].sigmag << ",";
	  outstream2 << fixed << setprecision(3) << outdetvec[i].sig_across << "," << outdetvec[i].sig_along << ",";
      ...
      outstream1 << fixed << setprecision(6) << outfile << " added " << tracklets_added << " tracklets totalling " << detections_added.size() << " points with RMS " << astromRMS << " and Kep a,e,i " << a << " " << e << " " << incl << "\n";
      cout << fixed << setprecision(6) << outfile << " added " << tracklets_added << " tracklets totalling " << detections_added.size() << " points\n";
      cout << fixed << setprecision(6) << "with RMS " << astromRMS << " and Kep a,e,i " << a << " " << e << " " << incl << "\n";
    }
  }
...
int add_tracklet02(const vector <vector <double>> &observer_statevecs, const vector <double> &obsMJD, const vector <double> &obsRA, const vector <double> &obsDec, const vector <double> &sigastrom, const vector <observatory> &observatory_list, const vector <double> &Earth_mjd, const vector <vector <double>> &Earth_statevecs, const vector <hldet> &outdetvec, const vector <long> &trkvec, const vector <hldet> &detvec, const vector <hlimage> &image_log, vector <vector <double>> &observer_statevecs2, vector <double> &obsMJD2, vector <double> &obsRA2, vector <double> &obsDec2, vector <double> &sigastrom2, vector <hldet> &outdetvec2, vector <long> &trkvec_temp, int verbose)
{
  ...
  while(obsct<long(obsMJD.size()) || tct<long(trkvec.size())) {
    if(obsct<long(obsMJD.size()) && (tct>=long(trkvec.size()) || obsMJD[obsct] < image_log[detvec[trkvec[tct]].image].MJD)) {
      ...
      sigastrom2.push_back(detvec[trkvec[tct]].sig_across);
