...
static void show_usage()
{
  ...
  cerr << "Note that in the unmatch invocation, it outputs lines from the FIRST file\nthat are NOT FOUND in the SECOND\n";
}
    
int main(int argc, char *argv[])
{
  string file1,file2,mstring,s1,s2,lnfromfile,idstring,outfile;
  int col1,col2;
  long linect1,linenum1,linect2,linenum2;
  linect1 = linect2 = linenum1 = linenum2 = 0;
  char128_index si1 = char128_index("",0);
  char128_index si2 = char128_index("",0);
  vector <char128_index> lines01;
  vector <char128_index> lines02;
  vector <char128_index> stringid01;
  vector <char128_index> stringid02;
  ifstream instream1;
  ofstream outstream1;
  int i=0;
  int badlines01=0;
  ...
  while(!instream1.eof() && !instream1.fail() && !instream1.bad()) {
	...
	si2 = char128_index(lnfromfile,linect1);
	lines01.push_back(si2);
	// Load string ID column into vector stringid01
	idstring = outstrings[col1-1];
	si1 = char128_index(idstring,linect1);
  ...
  while(!instream1.eof() && !instream1.fail() && !instream1.bad()) {
    ...
    if(!instream1.eof() && !instream1.fail() && !instream1.bad()) {
      ...
      stringnum = stringline01(lnfromfile, outstrings);
      if(stringnum>=col2) {
	si2 = char128_index(lnfromfile,linect2);
	lines02.push_back(si2);
	// Load string ID column into vector stringid02
	idstring = outstrings[col2-1];
	si1 = char128_index(idstring,linect2);
  ...
  sort(stringid01.begin(), stringid01.end(), lower_char128_index());
  sort(stringid02.begin(), stringid02.end(), lower_char128_index());
  ...
  if(mstring=="match") {
    ...
    while(linect1<linenum1 && linect2<linenum2) {
      s1 = char128_getstring(stringid01[linect1]);
      s2 = char128_getstring(stringid02[linect2]);
      if(s1==s2) {
	...
	dupline1=dupline2=0;
	s1 = char128_getstring(stringid01[linect1+1]);
	s2 = char128_getstring(stringid01[linect1]);
	if(linect1+1<linenum1 && s1==s2) dupline1=1;
	s1 = char128_getstring(stringid02[linect2+1]);
	s2 = char128_getstring(stringid02[linect2]);
	if(linect2+1<linenum2 && s1==s2) dupline2=1;
