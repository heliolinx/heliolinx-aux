// January 27, 2026: hldetTAI_2mpc.cpp
// Converts a file in hldet format to MPC80 format.
// Assumes there is no header, and that X05 observations
// are in TAI while all others are in UTC. Hence,
// TAI_MINUS_UTC = 37.0 seconds is subtracted from
// the times of X05 observations, but not the others.
...
static void show_usage()
{
  ...
  cerr << "OR\n";
  cerr << "Usage: hldet2mpc hldet_file\n";
}
...
int main(int argc, char *argv[])
{
  ...
  string signstring;
  char obscodeX05[MINSTRINGLEN];
  strcpy(obscodeX05, "X05");
  ...
  hldetfile=argv[1];
  if(argc==3) mpcfile=argv[2];

  cout.precision(17);  
  cout << "input observation file " << hldetfile << "\n";
  if(argc==3) cout << "output file " << mpcfile << "\n";
  ...
  // Correct the time stamps of any X05 observations
  for(i=0; i<obsnum; i++) {
    if(stringnmatch01(detvec[i].obscode,obscodeX05,3)==0) detvec[i].MJD -= TAI_MINUS_UTC/SOLARDAY;
  }
  ...
  if(argc==3) {
    ...
    for(i=0; i<obsnum; i++) {
      ...
      day = round(day*1000000.0l);
      day /= 1000000.0l;
      if(day<10.0l) cout  << fixed << setprecision(6) << "0";
      cout  << fixed << setprecision(6) << day;
      // Convert RA, Dec from decimal degrees to sexagesimal format.
      rahr = int(detvec[i].RA/15.0l);
      ramin = int(detvec[i].RA*4.0l - double(rahr)*60.0l);
      rasec = detvec[i].RA*240.0l - double(rahr)*3600.0l - double(ramin)*60.0l;
	...
	Dec = -detvec[i].Dec;
      ...
      if(ramin<10) cout << "0";
      cout << ramin << " ";
      if(rasec<10.0l) cout << "0";
      cout << fixed << setprecision(3) << rasec << signstring;
      if(decdeg<10) cout << "0";
      cout << decdeg << " ";
      if(decmin<10) cout << "0";
      cout << decmin << " ";
      if(decsec<10.0l) cout << "0";
      cout << fixed << setprecision(2) << decsec << "         ";
      // Write out magnitude and band.
      if(detvec[i].mag!=0.0) cout << fixed << setprecision(1) << detvec[i].mag << " ";
      else cout << "99.9 ";
      if(detvec[i].band[0]!='\0' && detvec[i].band[0]!=' ') {
	cout << detvec[i].band;
	// Determine number of spaces to add after band.
	...
	bandlen=1;
      ...
      for(j=0;j<7-bandlen;j++) cout << " ";
      // Write out obscode
      cout << detvec[i].obscode << "\n";
    }
  }    
