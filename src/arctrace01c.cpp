// January 29, 2025: arctrace01b.cpp
...
int main(int argc, char *argv[])
{
  ...
  point3LD(0,0,0);
  point3LD(0,0,0);
  planetposvel01LD(mjdstart-TTDELTAT/SOLARDAY, polyorder, planetmjd, Sunpos, Sunvel, outpos, outvel);
  startpos = point3LD(orbit[3]+outpos.x,orbit[4]+outpos.y,orbit[5]+outpos.z);
  startvel = point3LD(orbit[6]+outvel.x,orbit[7]+outvel.y,orbit[8]+outvel.z);
  ...
  chisq = tortoisechi02(polyorder,planetnum,planetmjd,planetmasses,planetpos,observerpos,obsMJD,obsRA,obsDec,sigastrom,startpos,startvel,mjdstart,fitRA,fitDec,fitresid);
  cout << "Chi-square value for input state vector is " << chisq << "\n";
  bestchi = chisq;
  bestRA = fitRA;
  bestDec = fitDec;
  ...
  while(chisq>1.0 && itct<ITMAX && pos_step>MINPOS_STEP && chifailct<=CHIFAILMAX) {
    ...
    orbgrad01a(chisq, pos_step, pvtimescale, polyorder, planetnum, planetmjd, planetmasses, planetpos, observerpos, obsMJD, obsRA, obsDec, sigastrom, startpos, startvel, mjdstart, gradient);
    ...
    newchi = orb1Dmin01a(chisq, poschange, pvtimescale, minchichange, thisway, polyorder, planetnum, planetmjd, planetmasses, planetpos, observerpos, obsMJD, obsRA, obsDec, sigastrom, startpos, startvel, mjdstart, fitRA, fitDec, fitresid, &beststep, tppos, tpvel);
  ...
  cout << fixed << setprecision(10) << "Best state vectors at MJD " << mjdstart << " : " << fixed << setprecision(3) << outpos.x << " " << outpos.y << " " << outpos.z << " "  << fixed << setprecision(10) << outvel.x << " " << outvel.y << " " << outvel.z << "\n";
