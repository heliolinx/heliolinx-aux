// February 28, 2025: MCMC_arctrace01c.cpp: Use an MCMC to fit
// an orbit to input data in hldet format.
...
{
  cerr << "Usage: MCMC_arctrace01c -cfg configfile -ranseed random_number_seed -observations obsfile -mjd starting_mjd -statevec x y z vx vy vz -maxnum maxnum -repnum repnum -obscode obscodefile -outfile outfile\n";
}
...
int main(int argc, char *argv[])
{
  ...
  for(itct=0;itct<maxnum;itct++) {
    ...
    for(i=0;i<6;i++) {
      ldval = unitvar(generator);
      tempstate[i] += eigenvecs[i][changepar]*(2.0*stepvec[changepar]*ldval - stepvec[changepar]);
    }
  ...
  for(i=0;i<long(accepted_all.size());i++) {
    statedist.push_back(sqrt(LDSQUARE(accepted_all[i][0]-beststate[0]) + LDSQUARE(accepted_all[i][1]-beststate[1]) + LDSQUARE(accepted_all[i][2]-beststate[2]) + LDSQUARE(accepted_all[i][3]-beststate[3]) + LDSQUARE(accepted_all[i][4]-beststate[4]) + LDSQUARE(accepted_all[i][5]-beststate[5])));
  }
  ...
  while(repct<repnum) {
    ...
    for(i=0;i<long(accepted_all.size());i++) {
      ldval = sqrt(LDSQUARE(accepted_all[i][0]-onestate[0]) + LDSQUARE(accepted_all[i][1]-onestate[1]) + LDSQUARE(accepted_all[i][2]-onestate[2]) + LDSQUARE(accepted_all[i][3]-onestate[3]) + LDSQUARE(accepted_all[i][4]-onestate[4]) + LDSQUARE(accepted_all[i][5]-onestate[5]));
