// February 06, 2025: arctrace02a.cpp
...
{
  cerr << "Usage: arctrace02a -cfg configfile -observations obsfile -kepspan time_span_for_Keplerian_fit(day) -minchi min_chi_change -mjdstart mjdstart -mjdend mjdend -outfile outfile -outfit outfit \n";
}
...
int main(int argc, char *argv[])
{
  ...
  int planetfile_startpoint,planetfile_endpoint;
  ...
  while(i<argc) {
	...
	cerr << "Output fit file keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
	...
	cerr << "Output ephemeris file keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
    } else {
  ...
  cout << "output fit file " << outfile << "\n";
  cout << "output ephmeris file " << outfile << "\n";
  ...
  arctrace01(polyorder,planetnum,planetmjd,planetmasses,planetpos,Sunpos,Sunvel,observerpos,obsMJD,obsRA,obsDec,sigastrom,kepspan,minchichange,startpos,startvel, bestRA, bestDec, bestresid, outpos, outvel, &bestchi, &astromrms, &refpoint);
  ...
  startpos = outpos;
  startvel = outvel;
  integrate_orbit05LD(polyorder, planetnum, planetmjd, planetmasses, planetpos, startpos, startvel, planetfile_startpoint, refpoint, planetfile_endpoint, orbit05MJD, orbit05pos, orbit05vel);
  
  outstream1.open(outfile);
  for(j=planetfile_startpoint; j<=planetfile_endpoint; j++) {
    int orbct=j-planetfile_startpoint;
    ...
    outpos.y /= ldval;
    outpos.z /= ldval;
    long double RA1,RA2,Dec1,Dec2,ravel,decvel;
    ...
    cout << "angdiffs: " << RA1 << " " << Dec1 << " " << RA2 << " " << Dec2 << " " << RA2-RA1 << " " << Dec2-Dec1 << "\n";
    ravel = (RA2-RA1)*cos(0.5*(Dec1+Dec2)/DEGPRAD)/TTDELTAT*SOLARDAY;
    decvel = (Dec2-Dec1)/TTDELTAT*SOLARDAY;
    outstream1 << fixed << setprecision(8) << orbit05MJD[orbct] << " " << RA1 << " " << Dec1 << " " << ravel << " " << decvel << " 20.0\n";
  }
  outstream1.close();
