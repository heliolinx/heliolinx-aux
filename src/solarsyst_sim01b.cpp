...
int main(int argc, char *argv[])
{
  ...
  for(simct=0; simct<simnum; simct++) {
   ...
   // Convert from AU and AU/day to km and km/sec
	...
	if(obsmag<limiting_mag  && (sundist/AU_KM)<=maxdetdist) {
	  ...
	  if(obsmag<limiting_mag) {
	    ...
	    if(dist<imrad) {
	      ...
	      outstream1 << planetmjd[planetfile_startpoint] << " " << outpos.x << " " << outpos.y << " " << outpos.z << " " << outvel.x << " " << outvel.y << " " << outvel.z << "\n";
	    } else {
	...
	outstream1 << targMJD[imct] << " " << targpos[imct].x  << " " << targpos[imct].y  << " " << targpos[imct].z  << " " << targvel[imct].x  << " " << targvel[imct].y  << " " << targvel[imct].z << "\n";
      }
      goodsimct++;
