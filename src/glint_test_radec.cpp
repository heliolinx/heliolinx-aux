// July 10, 2024: glint_test01.cpp
// Test the new glint_trail function
...
{
  cerr << "Usage: glint_test01 -dets detfile -minpoints minpoints -maxGCR maximum GRC -maxrange max range/ \n";
  cerr << "-cenRA image center RA -cenDec image center Dec -freq_downscale frequency downscale/ \n";
  cerr << "-freq_upscale frequency upscale -max_phase_err maximum phase error -trailout output trail file/ \n";
  cerr << "-trail2det output trail2det file\n";
  cerr << "\n    or, at minimum:\n";
  cerr << "glint_test01 -dets detfile\n";
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
    ...
    } else if(string(argv[i]) == "-cenRA") {
      if(i+1 < argc) {
	//There is still something to read;
        config.incenRA=stod(argv[++i]);
	config.centerknown++;
	i++;
      }
      else {
	cerr << "Image center RA keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
    } else if(string(argv[i]) == "-cenDec") {
      if(i+1 < argc) {
	//There is still something to read;
        config.incenDec=stod(argv[++i]);
	config.centerknown++;
	i++;
      }
      else {
	cerr << "Image center Dec keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
  ...
  if(config.centerknown>=2) config.centerknown=1; // Both RA and Dec were supplied
  else config.centerknown = 0;

  // Read detection file
  ifstream instream1 {indetfile};
  detct=0;
  while(!instream1.fail() && !instream1.bad() && !instream1.eof()) {
    instream1 >> RA >> Dec >> mag;
    if(!instream1.fail() && !instream1.bad() && !instream1.eof()) {
      onedet = point3d_index(RA,Dec,mag,detct);
  ...
  if(status!=0) {
    cerr << "ERROR: find_glints_radec failed with status " << status << "\n";
    return(status);
  }
  ...
  outstream1 << "#glintnum,RA,Dec,trail_len,trail_PA,GCR,eq_rms,magmean,magrms,stepsize,qc1,npt,flashnum\n";
