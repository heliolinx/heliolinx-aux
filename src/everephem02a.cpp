// June 22, 2026: everephem02a.cpp
// Given a file with a set of state vectors, of the format output by, e.g.,
// MCMC_everhart01a.cpp integrate all of the orbits and output the RA and Dec
// at a range of observation times.
...
{
  cerr << "Usage: everephem01a -cfg configfile -infile infile -obscode obscodefile -ephemcode ephemcode -mjdfile mjdfile -outfile outfile -verbose verbosity\n";
}
...
int main(int argc, char *argv[])
{
  ...
  while(i<argc) {
	...
	cerr << "Input MJD file keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
  ...
  // Read the input representative state vector file
  starting_statevecs = {};
  ...
  // Read the input MJD file: just a list of times at which to calculate the RA and Dec
  obsMJD = {};
  instream1.open(mjdfile);
  if(!instream1) {
    cerr << "can't open input mjd file " << mjdfile << "\n";
    return(1);
  }
  reachedeof = 0;
  while(reachedeof==0) {
    ...
    if(badread!=0) {
      cerr << "ERROR reading MJD file " << mjdfile << "\n";
      cerr << "Last point was " << obsMJD.size() << "; last file line was " << lnfromfile << "\n";
      return(badread);
    }
  }
  instream1.close();
  obsnum = obsMJD.size();
  cout << "Read " << obsnum << " lines from input MJD file " << mjdfile << "\n";
  
  // Calculate observer's barycentric position at every time in the MJD file
  eph_obsstate = {};
  minmjd = LARGERR2;
  maxmjd = 0.0;
  ...
  for(obsct=0;obsct<obsnum;obsct++) {
    ...
    cout << "Observer state vector at time " << obsMJD[obsct] << " is " << obsstate[0] << " " << obsstate[1] << " " << obsstate[2] << " " << obsstate[3] << " " << obsstate[4] << " " << obsstate[5] << "\n";
    eph_obsstate.push_back(obsstate);
    if(obsMJD[obsct]<minmjd) minmjd = obsMJD[obsct];
    if(obsMJD[obsct]>maxmjd) maxmjd = obsMJD[obsct];
  }
  if(long(eph_obsstate.size()) != obsnum) {
    cerr << "ERROR: obsMJD and obsstate vectors have different lengths: " << obsMJD.size() << " vs. " << eph_obsstate.size() << "\n";
    return(2);
  }
  
  // Calculate RA and Dec for all of the orbits at all of the times.
  outstream1.open(outfile);
  for(repct=0;repct<repnum;repct++) {
    one_statevec = starting_statevecs[repct];
    double a, e, incl;
    a = e = incl = 0;
    status = statevec2kep_easy(GMSUN_KM3_SEC2, one_statevec, a, e, incl);
    ...
    while(i>=hnum && (mjdstart>=minmjd-timestep || mjdstart>=mjdref-timestep)) {
      i-=hnum;
      mjdstart = planet_forward_mjd[i];
    }
    i=refpoint;
    mjdend = planet_forward_mjd[i];
    while(i<long(planet_forward_mjd.size()-hnum) && (mjdend<=maxmjd+timestep || mjdend<=mjdref+timestep)) {
      i+=hnum;
      mjdend = planet_forward_mjd[i];
    }
    targ_statevecs = {};
    ...
    for(obsct=0;obsct<obsnum;obsct++) {
      ...
      make_dvec(3,relpos);
      for(k=0;k<3;k++) relpos[k] = targ_statevecs[obsct][k] - eph_obsstate[obsct][k];
      ...
      for(k=0;k<3;k++) relpos[k] = targ_statevecs[obsct][k] - light_travel_time*targ_statevecs[obsct][3+k] - eph_obsstate[obsct][k];
      // Project onto the celestial sphere.
      statevec_to_celestial(relpos, RA1, Dec1);
      // Calculate the position a little bit later
      for(k=0;k<3;k++) relpos[k] += TTDELTAT*(targ_statevecs[obsct][3+k] - eph_obsstate[obsct][3+k]);
      // Project onto the celestial sphere.
      ...
      outstream1 << fixed << setprecision(6) << obsMJD[obsct] << " " << RA1 << " " << Dec1 << " " << RAvel << " " << Decvel << " " << repct << " " << obsct << " " << a/AU_KM << " " << e << " " << incl << "\n";
    }
  }
  outstream1.close();
  return(0);
