...
{
  cerr << "Usage: state2ephem01a -cfg configfile -mjdfile mjdfile -mjdstart mjdstart -mjdend mjdend -statevec x y z vx vy vz -obslon obslon -plxcos plxcos -plxsin plxsin -outfile outfile\n";
}
...
int main(int argc, char *argv[])
{
  ...
  integrate_orbit04LD(polyorder, planetnum, planetmjd, planetmasses, planetpos, startpos, startvel, planetfile_startpoint, planetfile_endpoint, targMJD, targpos, targvel);
  for(imct=0; imct < (planetfile_endpoint - planetfile_startpoint + 1); imct++) {
    outstream1 << fixed << setprecision(8) << targMJD[imct] << " ";
    outstream1 << fixed << setprecision(3) << targpos[imct].x  << " " << targpos[imct].y  << " " << targpos[imct].z  << " ";
    outstream1 << fixed << setprecision(8) << targvel[imct].x  << " " << targvel[imct].y  << " " << targvel[imct].z << "\n";
  }
  return(1);
