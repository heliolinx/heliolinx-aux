// February 24, 2025: MCMC_arctrace01a.cpp: Use an MCMC to fit
...
{
  cerr << "Usage: MCMC_arctrace01a -cfg configfile -ranseed random_number_seed -observations obsfile -mjd starting_mjd -statevec x y z vx vy vz -maxnum maxnum -obscode obscodefile -outfile outfile \n";
}
...
int main(int argc, char *argv[])
{
  ...
  for(itct=0;itct<maxnum;itct++) {
    ...
    if(newchi<=chisq) {
      ...
      outstream1 << fixed << setprecision(10) << planetmjd[planetfile_refpoint] << " " << fixed << setprecision(3) << startpos.x << " " << startpos.y << " " << startpos.z << " "  << fixed << setprecision(10) << startvel.x << " " << startvel.y << " " << startvel.z << " " << newchi << "\n";
