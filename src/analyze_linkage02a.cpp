...
int main(int argc, char *argv[])
{
  ...
  string spaceobsfile;
  string outfile,linkfile,colorfile;
  double mjdref = 0.0L;
  int configread=0;
  int planetnum=0;
  int planetct=0;
  ...
  vector <vector <double>> sunstatevec_obs;
  ...
  vector <double> bandoffsets;
  vector <vector <double>> band_resolved_objphot;
  ...
  while(i<argc) {
	...
	// Read the number of photometric bands
	status=readconfigint(instream1,&bandnum);
	while(status==1) {
	  // The line we have just read is a pure comment line,
	  // so we just want to skip to the next one.
	  status=readconfigint(instream1,&bandnum);
	...
	if(status<0) {
	  cerr << "Error reading config file\n";
	  return(1);
	} else cout << "Number of photometric bands read as " << bandnum << "\n";
	for(bandct=0; bandct<bandnum; bandct++) {
	  // Read the string identifier (probably just one character) for this photometric band
	  status=readconfigstring(instream1,bandstring);
	  while(status==1) {
	    // The line we have just read is a pure comment line,
	    // so we just want to skip to the next one.
	    status=readconfigstring(instream1,bandstring);
	  ...
	  if(status<0) {
	    ...
	    cout << "Photometric band " << bandct+1 << " is named " << bandstring << "\n";
	    bandnames.push_back(bandstring);
	  }
	}
	for(bandct=0; bandct<bandnum; bandct++) {
	  // Read the photometric offset (i.e., mean asteroid color relative to the reference band) for this photometric band
	  status=readconfigd(instream1,&ldval);
	  while(status==1) {
	    // The line we have just read is a pure comment line,
	    // so we just want to skip to the next one.
	  ...
	  if(status<0) {
	    ...
	    cout << "Offset for photometric band " << bandct+1 << "(" << bandnames[bandct] << ") is " << ldval << " magnitudes\n";
	    bandoffsets.push_back(ldval);
	  }
	}
    ...
    if(configread!=1) {
      ...
      return(2);
    } else if(int(bandnames.size())!=bandnum || int(bandoffsets.size())!=bandnum) {
      cerr << "ERROR: mismatch in lengths of config vectors specifying photometric bands: " << bandnum << " " << bandnames.size() << " " << bandoffsets.size() << "\n";
      return(3);
    ...
    } else if(string(argv[i]) == "-colorfile" || string(argv[i]) == "-color") {
      if(i+1 < argc) {
	//There is still something to read;
	colorfile=argv[++i];
	i++;
	...
	cerr << "Color file keyword supplied with no corresponding argument\n";
	show_usage();
	return(1);
      }
  ...
  for(obsct=0;obsct<obsnum;obsct++) {
    ...
    sunstatevec_obs.push_back(sunstate);
  ...
  out_statevec={};
  status = evertrace02(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, starting_statevec, mjdref, obsMJD, observer_statevecs, obsRA, obsDec, sigastrom, fitRA, fitDec, out_statevec, targ_statevecs, timestep, hnum, hspace, minchichange, astromrmsthresh, maxiter, itnum, astromrms, chisq, verbose);
  if(status!=0) {
    cerr << "ERROR: evertrace02 returned error status " << status << "\n";
    return(status);
  }
  // Use the best-fit state vectors in targ_statevecs to calculate the phase at every point.  
  // Model the expected brightness variations, and resolve by band
  // Create empty photometry arrays
  band_median_objphot = {};
  ...
  for(bandct=0;bandct<bandnum;bandct++) {
    ldval = -99.9;
    if(band_resolved_objphot[bandct].size()>0) ldval = dmedian(band_resolved_objphot[bandct]);
    band_median_objphot.push_back(ldval);
  }
  // Write out colors
  outstream1.open(colorfile);
  for(bandct=0;bandct<bandnum;bandct++) {
    for(i=bandct+1;i<bandnum;i++) {
      if(bandct==0 && i==1) outstream1 << "band" << bandct << "-band" << i;
      else  outstream1 << ",band" << bandct << "-band" << i;
    }
  }
  outstream1 << "\n";
  for(bandct=0;bandct<bandnum;bandct++) {
    for(i=bandct+1;i<bandnum;i++) {
      if(band_median_objphot[bandct] <= -99.0 || band_median_objphot[i] <= -99.0) {
	// There is no valid photometry in one of the bands. Print dummy value.
	if(bandct==0 && i==1) outstream1 << "-99.9";
	else outstream1 << ",-99.9";
      } else {
	if(bandct==0 && i==1) outstream1 << band_median_objphot[bandct] - band_median_objphot[i];
	else outstream1 << "," << band_median_objphot[bandct] - band_median_objphot[i];
      }
    }
  }
  outstream1 << "\n";
