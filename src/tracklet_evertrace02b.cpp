// January 22, 2026: tracklet_evertrace02b.cpp:
...
int main(int argc, char *argv[])
{
  ...
  while(!instream2.fail() && !instream2.bad() && !instream2.eof()) {
	...
	KepfitRA = KepfitDec = Kepfitresid = {};	
	chisq = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_statevecs, obsMJD, obsRA, obsDec, sigastrom, KepfitRA, KepfitDec, Kepfitresid, kep_statevec, stateMJD, itnum, verbose);
	Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, mjd_sv_vec[0], out_statevec, verbose);
	astromRMS = 0.0;
	for(i=0;i<long(obsMJD.size());i++) astromRMS += Kepfitresid[i]*Kepfitresid[i];
	astromRMS  = sqrt(astromRMS / double(obsMJD.size()-1));
	...
	if(trkvec_temp.size()>0) {
	  ...
	  if(status!=0) {
	    ...
	    outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	    bestRA = bestDec = bestresid = {};
	    bestchi = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_statevecs2, obsMJD2, obsRA2, obsDec2, sigastrom2, bestRA, bestDec, bestresid, kep_statevec, stateMJD, itnum, verbose);
	    Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, master_mjd, out_statevec, verbose);
	    astromRMS = 0.0;
	    for(i=0;i<long(obsMJD.size());i++) astromRMS += bestresid[i]*bestresid[i];
	    astromRMS  = sqrt(astromRMS / double(obsMJD2.size()-1));
	...
	for(twinfitct=0;twinfitct<twinfitnum;twinfitct++) {
	  ...
	  for(matchct=twinfitct+1;matchct<allfitnum;matchct++) {
		...
		outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
		bestRA = bestDec = bestresid = {};
		bestchi = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_statevecs3, obsMJD3, obsRA3, obsDec3, sigastrom3, bestRA, bestDec, bestresid, kep_statevec, stateMJD, itnum, verbose);
		Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, master_mjd, out_statevec, verbose);
		astromRMS = 0.0;
		for(i=0;i<long(obsMJD.size());i++) astromRMS += bestresid[i]*bestresid[i];
		astromRMS  = sqrt(astromRMS / double(obsMJD3.size()-1));
		cout << "\nRescue Keplerian fit returns chisq, RMS = " << bestchi << " " << astromRMS << "\n";
		outstream1 << "Rescue Keplerian fit returns chisq, RMS = " << bestchi << " " << astromRMS << "\n";
	      } else {
		// Calculate residuals from best-fit
	...
	outstream1 << "Launching Keplerian fit with geodists " << geodist1 << " and " << geodist2 << "\n";
	bestRA = bestDec = bestresid = {};
	bestchi = Hergetfit_vstarSV(geodist1, geodist2, simplex_scale, simptype, ftol, 1, kepnum, observer_statevecs, obsMJD, obsRA, obsDec, sigastrom, bestRA, bestDec, bestresid, kep_statevec, stateMJD, itnum, verbose);
	Kepler_univ_int_SV(GMSUN_KM3_SEC2, stateMJD, kep_statevec, master_mjd, out_statevec, verbose);
	astromRMS = 0.0;
	for(i=0;i<long(obsMJD.size());i++) astromRMS += bestresid[i]*bestresid[i];
	astromRMS  = sqrt(astromRMS / double(obsMJD2.size()-1));
