...
int main(int argc, char *argv[])
{
  ...
  double lambda,phi,x,y,sunelong;
  lambda = phi = x = y = sunelong = 0l;
  ...
  for(detct=0 ; detct<detnum ; detct++) {
    ...
    sunelong = distradec01(sunRA,sunDec,indets[detct].RA, indets[detct].Dec);
    ...
    outstream1 << fixed << setprecision(6) << x << " " << y << " " << sunelong << "\n";
  }
  outstream1.close();
  return(0);
}
