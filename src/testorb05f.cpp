...
int main(int argc, char *argv[])
{
  ...
  vector <vector <double>> testtarg_statevecs;
  vector <double> starting_statevec;
  vector <double> test_statevec;
  ...
  vector <double> Xcor;
  vector <vector <double>> test_fitRA;
  vector <vector <double>> test_fitDec;
  double pos_step = 100.0;
  double vel_step = 0.01;
  ...
  test_fitRA = test_fitDec = {};
  long l=0;
  for(l=0;l<3;l++) {
    test_statevec = starting_statevec;
    test_statevec[l]+=pos_step;
    obsint_vareq01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, test_statevec, mjdstart, mjdref, mjdend, obsTDB, targ_statevecs, vareq_mat, timestep, hnum, hspace, 0);
    for(i=0;i<obsnum;i++) testtarg_statevecs.push_back(targ_statevecs[i]);
    fitRA = fitDec = {};
    for(i=0; i<obsnum; i++) {
    ...
    test_fitRA.push_back(fitRA);
    test_fitDec.push_back(fitDec);
  }
  for(l=3;l<6;l++) {
    test_statevec = starting_statevec;
    test_statevec[l]+=vel_step;
    obsint_vareq01(planetnum, planetmasses, planet_backward_mjd, planet_backward_statevecs, planet_forward_mjd, planet_forward_statevecs, test_statevec, mjdstart, mjdref, mjdend, obsTDB, targ_statevecs, vareq_mat, timestep, hnum, hspace, 0);
    for(i=0;i<obsnum;i++) testtarg_statevecs.push_back(targ_statevecs[i]);
    fitRA = fitDec = {};
    for(i=0; i<obsnum; i++) {
  ...
  for(i=0; i<obsnum; i++) {
    ...
    cout << scientific << "Analytical RA derivatives: " << RA_deriv_mat[i][0] << " " << RA_deriv_mat[i][1] << " " << RA_deriv_mat[i][2] << "\n";
    cout << scientific << "Analytical Dec derivatives: " << Dec_deriv_mat[i][0] << " " << Dec_deriv_mat[i][1] << " " << Dec_deriv_mat[i][2] << "\n";
    double newRA,newDec;
    double delta_pos = 100.0;
    ...
    cout << scientific << "Numerical Dec derivatives: " << Dec_deriv[0] << " " << Dec_deriv[1] << " " << Dec_deriv[2] << "\n";

    cout << "Analytical variational derivatives: \n";
    // These are derivatives of the final quantities w.r.t. the initial quantities.
    // k indexes the final quantities, which are printed all the same in a given row.
    // l indexes the initial quantities, which are printed the same in a given column.
    for(k=0;k<6;k++) {
      for(l=0;l<6;l++) cout << vareq_mat[i][k*6+l] << " ";
      cout << "\n";
    }
    cout << "Numerical variational derivatives: \n";
    for(k=0;k<6;k++) {
      for(l=0;l<3;l++) cout << (testtarg_statevecs[l*obsnum+i][k] -  targ_statevecs[i][k])/pos_step << " ";
      for(l=3;l<6;l++) cout << (testtarg_statevecs[l*obsnum+i][k] -  targ_statevecs[i][k])/vel_step << " ";
      cout << "\n";
    }
  }
  ...
  cout << "Aobs_mat, RA: \n";
  for(i=0;i<obsnum;i++) {
    for(j=0;j<6;j++) cout << scientific << " " << Aobs_mat[2*i][j];
    cout << "\n";
  }
  cout << "Numerical derivatives, RA: \n";
  for(i=0;i<obsnum;i++) {
    for(j=0;j<3;j++) cout << scientific << " " << (test_fitRA[j][i] - fitRA[i])/pos_step;
    for(j=3;j<6;j++) cout << scientific << " " << (test_fitRA[j][i] - fitRA[i])/vel_step;
    cout << "\n";
  }

  cout << "Aobs_mat, Dec: \n";
  for(i=0;i<obsnum;i++) {
    for(j=0;j<6;j++) cout << scientific << " " << Aobs_mat[2*i+1][j];
    cout << "\n";
  }

  cout << "Numerical derivatives, Dec: \n";
  for(i=0;i<obsnum;i++) {
    for(j=0;j<3;j++) cout << scientific << " " << (test_fitDec[j][i] - fitDec[i])/pos_step;
    for(j=3;j<6;j++) cout << scientific << " " << (test_fitDec[j][i] - fitDec[i])/vel_step;
    cout << "\n";
  }
  return(0);
  Aobs_transpose = {};
